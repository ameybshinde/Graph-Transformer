...
from GraphTransformer.UGformerV1_TF import universal_transformer_modified, universal_transformer_modified_utils
...
class ActionPredict(object):
    ...
    def train(self, data_train,
        ...
        print(f"Training input data : {data_train['data'][0]}")
        print(f"Training output data : {data_train['data'][1]}")
    ...
    def test(self, data_test, model_path=''):
        ...
        print(f"Loading model configuration from {os.path.join(model_path, 'configs.yaml')}")
        ...
        model = os.path.join(model_path, 'model.h5')
        # test_model = load_model(model)
        test_model = load_model(model, custom_objects={"Encoder": Encoder})
        print(f"Model: {test_model}")
        # test_model = load_model(os.path.join(model_path, 'model.h5'), custom_objects={"Encoder": Encoder}, safe_mode = False)
        # test_model.summary()
        # Inspecting model's input layer
        input_layer = test_model.layers[0]
        ...
        test_model.summary()
        # print(f"Test Seq data: {data_test}")'data_sizes': [(16, 112, 112, 3), (16, 4)]}
        test_data = self.get_data('test', data_test, {**opts['model_opts'], 'batch_size': 1})
        print(f"Test data: {test_data}")
        print("Test_data['data'][0][0]:", test_data['data'][0][0])
        print("Test_data['data'][0][0][0]:", test_data['data'][0][0][0])

        print("Shape of Test_data['data'][0][0]:", len(test_data['data'][0][0]))
        print("Shape of Test_data['data'][0][0][0]:", len(test_data['data'][0][0][0]))


        # print("Shape of Test_data['data'][0]:", test_data['data_params']['data_sizes'])
        # print("Shape of test_data['data'][0]:", test_data['data'][0].__getitem__(index=0))
        # print("Length of test_data['data'][0]:", test_data['data'][0].__getitem__(index=0).__len__())
        # print("Shape of test_data['data'][0]:", test_data['data'][0].__getitem__(index=1))
        # print("Length of test_data['data'][0]:", test_data['data'][0].__getitem__(index=1).__len__())
        # print("Shape of test_data['data'][0]:", test_data['data'][0].__getitem__(index=2))
        # print("Length of test_data['data'][0]:", test_data['data'][0].__getitem__(index=2).__len__())


        # # Data generator
        # # Create an instance of the DataGenerator class
        # data_generator = DataGenerator(
        #     data=data_test,  # Your data input
        #     labels=labels,  # Your labels
        #     data_sizes=data_sizes,  # Your data sizes
        #     process=True,  # Process data (adjust as needed)
        #     global_pooling='max',  # Global pooling type (adjust as needed)
        #     input_type_list=input_type_list,  # List of input types
        #     batch_size=32,  # Batch size
        #     shuffle=True,  # Shuffle data
        #     to_fit=True,  # Whether to generate data for training or prediction
        #     stack_feats=False  # Stack features (adjust as needed)
        # )

        # data = test_data['data'][0]
        
        # X_data_1 = data.__getitem__(0)
        # # X_data_1 = X_data[0]
        # # X_data_2 = X_data[1]
        # X_data_2 = data.

        # data2 = test_data['data'][1]

        # y_data = data2.__getitem__(0)
        # y_data_1 = y_data[0]
        # y_data_2 = data2 
        # test_results = test_model.predict([X_data_1,X_data_2],
        #                                  batch_size=1, verbose=1)
        # print(f"Test Results: {test_results}")


        print("test_data['data'][1][0]:", test_data['data'][1][0])

        print("test_data['data'][1]:", test_data['data'][1])
        print("Shape of test_data['data'][1]:", len(test_data['data'][1]))


        test_results = test_model.predict(test_data['data'][0][0],
                                          batch_size=8, verbose=1)
        # acc = accuracy_score(y_data, np.round(test_results))
        # print(f"Accuracy: {acc}")

        print("len test_data['data'][1][0]:", len(test_data['data'][1][0]))  
        print(f"Results: {np.round(test_results)}")

        print(f"Results: {test_results[0]}")
        
        

        true_labels = test_data['data'][1]
        predicted_value = test_results[0][0]
        # predicted_labels = np.full_like(true_labels, predicted_value)
        
        # Set a custom threshold (e.g., 0.4)
        custom_threshold = 0.4
        print("Custom Threshold", custom_threshold)
        custom_predictions = (test_results >= custom_threshold).astype(int)
        predicted_labels = np.full_like(true_labels, custom_predictions)

                # Calculate precision and recall using the adjusted threshold
        ...
        print("Lengeth of predicted labels:", len(predicted_labels))
        # print("test_data['data'][1][0]:", len(test_data['data'][1][0][0]))
        # print("test_data['data'][1][0]:", len(test_data['data'][1][1][1]))
        # print("test_data['data'][1][0]:", len(test_data['data'][1]))
        # print("test_data['data'][1][0][0]:", len(test_data['data'][1][0][0]))
        acc = accuracy_score(true_labels, np.round(predicted_labels))
        # acc = accuracy_score(test_data['data'][1], np.round(test_results).ravel())
        print(f"Accuracy: {acc}")
        ...
        f1 = f1_score(true_labels, np.round(predicted_labels))
        try:
            auc = roc_auc_score(true_labels, np.round(predicted_labels))
        except ValueError:
            auc=0
        roc = roc_curve(true_labels, np.round(predicted_labels))
        precision = precision_score(true_labels, np.round(predicted_labels))
        recall = recall_score(true_labels, np.round(predicted_labels))
        pre_recall = precision_recall_curve(true_labels, np.round(predicted_labels))
...
# class CAPformerWithGraphTransformer(ActionPredict):
#     def __init__(self, num_hidden_units=256, cell_type='gru', num_graph_features, num_classes, **kwargs):
#         super().__init__(**kwargs)
#         self._num_hidden_units = num_hidden_units
#         self._rnn = self._gru if cell_type == 'gru' else self._lstm
#         self._rnn_cell = GRUCell if cell_type == 'gru' else LSTMCell
...
#         # Graph Transformer
#         self.graph_transformer = UGformerV1(
#             feature_dim_size=num_graph_features,
#             hparams_batch_size=self._batch_size,
#             ff_hidden_size=512,  # Adjust this value as needed
#             seq_length=self._obs_length,
#             num_classes=num_classes,  # Update with the number of classes in your task
#             num_self_att_layers=4,  # Adjust this value as needed
#             num_GNN_layers=1  # Adjust this value as needed
#         )
...
class CAPformer(ActionPredict):
    ...
    """
    def __init__(self, num_graph_features=2, num_classes=1, num_hidden_units=256, cell_type='gru', **kwargs):
        super().__init__(**kwargs)
        self._num_hidden_units = num_hidden_units
        self._rnn = self._gru if cell_type == 'gru' else self._lstm
        self._rnn_cell = GRUCell if cell_type == 'gru' else LSTMCell
        ...
        # self.num_graph_features = [(16, 112, 112, 3), (16, 4)]
        self.num_graph_features = 2
        self.num_classes=1
    ...
    def get_model(self, data_params):
        ...
        # Check input shapes
        # print("Input shapes:")
        # print("Input 1 shape:", net_model.input[0].shape)
        # print("Input 2 shape:", net_model.input[1].shape)
        
        
        # Add the Graph Transformer
        # graph_input = tf.compat.v1.placeholder(tf.float32, [None, self.num_classes, self.num_graph_features], name="graph_input")
        graph_input = tf.Variable(initial_value=tf.zeros((1, self.num_classes, self.num_graph_features), dtype=tf.float32), trainable=True, name="graph_input")
        graph_transformer_output = self.graph_transformer(graph_input)

        # Concatenate Features
        combined_features = tf.concat([encodings, graph_transformer_output], axis=-1)

        # Continue with the rest of the model

        # Define the final output layer for the combined features
        model_output = layers.Dense(1, activation='sigmoid', name='output_dense')(combined_features)
...
class UGformerV1(tf.Module):
    def __init__(self, feature_dim_size, seq_length, num_classes, num_self_att_layers, num_GNN_layers=1):
        super(UGformerV1, self).__init__()
        self.seq_length = seq_length
        self.num_GNN_layers = num_GNN_layers
        
        # Universal Transformer hyperparameters
        self.hparams = tf.keras.optimizers.schedules.PolynomialDecay(
            initial_learning_rate=0.01,
            decay_steps=1000,
            end_learning_rate=0.001,
        )
        
        self.ute = tf.keras.layers.MultiHeadAttention(num_heads=1, key_dim=2)
        self.dense_layer = Dense(units=num_classes, activation='linear')

    def call(self, input_x, graph_pool, X_concat, one_hot_labels, dropout_keep_prob):
        # Inputs for Universal Transformer
        input_UT = tf.gather(X_concat, input_x)
        input_UT = tf.reshape(input_UT, [-1, self.seq_length, 1, input_UT.shape[-1]])
        
        # Construct k GNN layers
        scores = 0
        for layer in range(self.num_GNN_layers):
            # Universal Transformer Encoder
            output_UT = self.ute(input_UT, input_UT, input_UT)
            output_UT = tf.squeeze(output_UT, axis=2)
            output_target_node = output_UT[:, :1, :]
            output_target_node = tf.squeeze(output_target_node, axis=1)

            # Input for the next GNN hidden layer
            input_UT = tf.gather(output_target_node, input_x)
            input_UT = tf.reshape(input_UT, [-1, self.seq_length, 1, input_UT.shape[-1]])

            # Graph pooling
            graph_embeddings = tf.sparse.sparse_dense_matmul(graph_pool, output_target_node)
            graph_embeddings = tf.nn.dropout(graph_embeddings, rate=1.0 - dropout_keep_prob)

            # Concatenate graph representations from all GNN layers
            scores += self.dense_layer(graph_embeddings)
        
        # Final predictions
        predictions = tf.argmax(scores, axis=1, output_type=tf.dtypes.int32)
        
        # Calculate mean cross-entropy loss
        cce = CategoricalCrossentropy(from_logits=True)
        loss = cce(one_hot_labels, scores)
        
        # Accuracy
        accuracy = Accuracy()
        accuracy.update_state(tf.argmax(one_hot_labels, axis=1, output_type=tf.dtypes.int32), predictions)
